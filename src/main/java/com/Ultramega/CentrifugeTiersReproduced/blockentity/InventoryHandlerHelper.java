...
public class InventoryHandlerHelper {
    ...
    public static final int[] OUTPUT_SLOTS = new int[]{5, 6, 7, 8, 9, 10, 11, 12, 13, /*Cosmic*/ 14, 15, 16, /*Creative*/ 17, 18, 19, 20};

    private static int getAvailableOutputSlot(ItemHandler handler, ItemStack insertStack, CentrifugeTiers tier) {
        return getAvailableOutputSlot(handler, insertStack, new ArrayList<>(), tier);
    }
    ...
    public static class ItemHandler extends ItemStackHandler {
        ...
        public boolean isInputSlot(int slot) {
            return slot == BOTTLE_SLOT || slot == INPUT_SLOT[0] || slot == INPUT_SLOT[1] || slot == INPUT_SLOT[2] || slot == INPUT_SLOT[3];
        }

        public boolean isInsertableSlot(int slot) {
            return slot != BOTTLE_SLOT && slot != INPUT_SLOT[0] && slot != INPUT_SLOT[1] && slot != INPUT_SLOT[2] && slot != INPUT_SLOT[3] && slot != FLUID_ITEM_OUTPUT_SLOT;
        }
        ...
        public boolean addOutput(@Nonnull ItemStack stack) {
            int slot = getAvailableOutputSlot(this, stack, ((TieredCentrifugeBlockEntity)tileEntity).tier);
            if (slot > 0) {
        ...
        public boolean canFitStacks(List<ItemStack> stacks) {
            ...
            for (ItemStack stack : stacks) {
                int slot = getAvailableOutputSlot(this, stack, usedSlots, ((TieredCentrifugeBlockEntity)tileEntity).tier);
                if (slot == 0) {
                    return false;
                }
            }
